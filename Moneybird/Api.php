...
class MoneybirdApi {
	...
	public function typeInfo($type) {
		switch ($type) {
            ...
            case 'ledgerAccount':
                return array('ledger_accounts', 'MoneybirdLedgerAccount');
                break;
	...
	protected function request($url, $method='GET', iMoneybirdObject $mbObject=null, iMoneybirdObject $parent=null) {
		...
		switch ($method) {
			...
			case 'PUT':
				...
				$curlopts[CURLOPT_HTTPHEADER][] = 'Content-Length: ' . strlen($xml);*/
		...
		if (isset($error)) {
			if (
				...
				$this->errors = array();
                foreach ($xmlresponse->error as $message) {
	...
	public function getContactByCustomerId($contactID) {
		...
		$response = $this->request($typegroup . '/' . $contactID);
    ...
    public function getLedgerAccounts() {
        return $this->getMbObjects('ledgerAccount');
    }
